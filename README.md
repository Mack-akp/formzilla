...
-   `storage`: Where to store the files, if any, included in the request. Formzilla provides the following built-in options. It is possible to write custom storage plugins of your own.
    ...
    -   `BufferStorage`: Emulates Formzilla 1.x behaviour by storing file contents as a `Buffer` in the `data` property of the file. Example:
        ...
        ```
    -   `DiscStorage`: Saves the file to the disc. Accepts parameters that can be either a `formzilla.FileSaveTarget` or a function that accepts a `formzilla.File` parameter and returns a `formzilla.FileSaveTarget` and 2nd parameter to save persistent files, default is false. By default, Formzilla will save the file to the operating system's TEMP directory and clear it in `OnResponse Hook`. Example:
        ```tsx
        server.register(formDataParser, {
